...
#### Centering inline/inline-block Child Elements

`text-align` applies to block containers and specifies how inline child elements like `<img>`, `<input>`, text content or child elements with `display: inline-block` is aligned in parent block container. This property has no effect on block child elements.
...
#### Inline or Table Content
 ...
 [`vertical-align`](https://developer.mozilla.org/en-US/docs/Web/CSS/vertical-align) applies to inline-level and table-cell elements. It aligns boxes inside a line box generated by inline level element vertically. Notice it cannot align content of `inline-block` since it has block formatting context instead of inline formatting context inside it.
