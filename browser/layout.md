...
1. [Round-up of web browser internals resources](https://developer.chrome.com/blog/round-up-of-web-browser-internals-resources/)
...
1. [Flutter Constraints](https://docs.flutter.dev/ui/layout/constraints)
1. [Simplifying WebGL: Building an Effective Layout Engine](https://blog.certa.dev/building-a-layout-engine-for-webgl)
1. [网页布局简史](https://zhuanlan.zhihu.com/p/104927765)
1. [How browsers work | The Rendering Engine](https://scientyficworld.org/how-browsers-work-the-rendering-engine/)
1. [Learn CSS Layout](https://book.mixu.net/css/)
1. [Faster HTML and CSS: Layout Engine Internals for Web Developers](https://www.youtube.com/watch?v=a2_6bGNZ7bA&t=1411s)
1. [Fast CSS: How Browsers Lay Out Web Pages](https://dbaron.org/talks/2012-03-11-sxsw/master.xhtml)
1. [An Ultimate Guide on Browser Fundamentals| Rendering Engine](https://www.cloudthat.com/resources/blog/an-ultimate-guide-on-browser-fundamentals-rendering-engine)

## 盒子模型 Box Model

## Box Generation

[Controlling Box Generation](https://www.w3.org/TR/CSS2/visuren.html#box-gen)

## 流式布局 Flow ( Block & Inline)

[流式布局](https://developer.mozilla.org/zh-CN/docs/Web/CSS/display#css_flow_layout_display_block_display_inline)

### Margins & Paddings Calculation
...
margin-left/margin-right 初始值是 0 ，是只有在 auto 的情况下才会被重新计算么，为负值？

1. margin-left/margin-right auto 的情况下才能被调整，可以为负值
2. width: auto 可以被调整，不能为负值
3. 被调整的优先级 margin-left/margin-right。

4. overflow 的情况下，没有剩余空间可以分配
   1. width: auto 等于 0，其余的 auto 值也为 0，但是 margin-right 要重新计算值使得约束成立
   1. width 不是 auto 的情况，因为 width 本身造成了 overflow，其余 auto 为 0，不需要重新计算使约束成立
5. underflow 的情况下，存在剩余空间可以分配
   1. width: auto 占据全部剩余空间，其余 auto 为 0
   1. width: 不是 auto 的情况，margin-left/margin-right 有一个 auto 时占据剩余空间，两个都是 auto 时评分剩余空间，都不是 auto 时剩余空间分配给 margin-right。
6. 上述计算的是 preferred width，另外要考虑到 min-width/max-width 确定最后真实的 width。
